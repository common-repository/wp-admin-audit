...
class WADA_Sensor_Plugin extends WADA_Sensor_Base
{
    ...
    public function onAdminInit(){
        ...
        $isUpdate = self::matchEvent(array('action' => 'update'), $_REQUEST) || self::matchEvent(array('action' => 'update-selected'), $_REQUEST);
        ...
        foreach($allPlugins AS $pluginFile => $plugin){
            $pluginBefore = (object)($this->extractPluginData($plugin, array('Version', 'Name', 'Title', 'PluginURI', 'Author')));
     ...
     * @return array
     */
    protected function extractPluginData($plgData, $keys) {
        $result = array();
        foreach($keys as $key){
            $result[$key] = array_key_exists($key, $plgData) ? $plgData[$key] : null;
        }
        return $result;
    }
    ...
    public function onPluginInstall($upgrader, $hookExtract){
        ...
        if($res->overwrite === 'update-plugin' || $res->overwrite === 'downgrade-plugin'){ // we went into it as an installation, but it turns out the plugin already exists -> go for plugin update event instead
            ...
            WADA_Log::debug('onPluginInstall -> is update -> overwrite active: '.$res->overwrite);
            WADA_Log::debug('onPluginInstall -> is update -> pluginPathForSearch: '.$pluginPathForSearch);
            WADA_Log::debug('onPluginInstall -> is update -> pluginsBefore: '.print_r($this->pluginsBefore, true));
    ...
    public function onPluginUpdate($upgrader, $hookExtract){
        ...
        if(array_key_exists('plugins', $hookExtract)) {
            foreach ($hookExtract['plugins'] AS $plg){
                if(count($hookExtract['plugins']) > 1){
                ...
                $plgDataBefore = array_key_exists($plg, $this->pluginsBefore) ? ((array)$this->pluginsBefore[$plg]) : null;

                $keys = array('Version', 'Name', 'Title', 'PluginURI', 'Author');
                $plgDataCurrent = $plgData ? $this->extractPluginData($plgData, $keys) : array_fill_keys($keys, null);
                $plgDataBefore = $plgDataBefore ? $this->extractPluginData($plgDataBefore, $keys) : array_fill_keys($keys, null);

                $res->infos[] = self::getEventInfoElement('PLUGIN_VERSION', $plgDataCurrent['Version'], $plgDataBefore['Version']);
                $res->infos[] = self::getEventInfoElement('Name', $plgDataCurrent['Name'], $plgDataBefore['Name']);  // always record name even if unchanged
                $res->infos[] = self::getEventInfoElementIfChanged('PLUGIN_TITLE', $plgDataCurrent['Title'], $plgDataBefore['Title']);
                $res->infos[] = self::getEventInfoElementIfChanged('PLUGIN_URL', $plgDataCurrent['PluginURI'], $plgDataBefore['PluginURI']);
                $res->infos[] = self::getEventInfoElementIfChanged('PLUGIN_AUTHOR', $plgDataCurrent['Author'], $plgDataBefore['Author']);

                $storeRes = $this->storePluginEvent(self::EVT_PLUGIN_UPDATE, array('infos' => $res->infos)); // trigger one event per plugin
            }
        }
        return $storeRes;
    }
