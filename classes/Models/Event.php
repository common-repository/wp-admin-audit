    ...
    protected function storeEventInfos($eventId, $eventInfos){
        if($eventId > 0 && is_array($eventInfos) && count($eventInfos)>0){
            ...
            foreach($eventInfos AS $eventInfoArr) {
                if(is_null($eventInfoArr)){
                    WADA_Log::warning('storeEventInfos skip empty entry for eventId '.$eventId.' eventInfos (all) are: '.print_r($eventInfos, true));
                    continue; // skip empty
                }

                $infoObj = (object)$eventInfoArr;

                if(is_null($infoObj) || !property_exists($infoObj, 'info_key')){
                    WADA_Log::warning('storeEventInfos skip empty entry for eventId '.$eventId.' eventInfos (all) are: '.print_r($eventInfos, true));
                    continue; // skip empty
                }
