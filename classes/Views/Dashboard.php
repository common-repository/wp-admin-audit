                ...
                <div class="wada-welcome-panel-content">
                    ...
                    <div class="wada-welcome-panel-column-container">
                        <div class="wada-welcome-panel-column">
                            ...
                            <?php
                            $nrPossibleExtensions = count($this->_stats['possible_extensions']);

                            /* @@REMOVE_START_WADA_enterprise@@ */
                            /* @@REMOVE_START_WADA_business@@ */
                            /* @@REMOVE_START_WADA_startup@@ */

                            if($nrPossibleExtensions):
                                $extensionText = '';
                                $upgradeEdition = WADA_Version::getMinV4Ft(WADA_Version::FT_ID_EXT);
                                $targetPlg = $this->_stats['possible_extensions'][0]->target_plugin;
                                if($nrPossibleExtensions == 1){
                                    $extensionText = sprintf(__('Upgrade to <strong>%1$s</strong> to capture <strong>%2$s</strong> events', 'wp-admin-audit'), $upgradeEdition, $targetPlg);
                                }elseif($nrPossibleExtensions == 2){
                                    $extensionText = sprintf(__('Upgrade to <strong>%1$s</strong> to capture <strong>%2$s</strong> events (and for at least one more plugin)', 'wp-admin-audit'), $upgradeEdition, $targetPlg);
                                }elseif($nrPossibleExtensions > 2){
                                    $extensionText = sprintf(__('Upgrade to <strong>%1$s</strong> to capture <strong>%2$s</strong> events (and for at least %d other plugins)', 'wp-admin-audit'), $upgradeEdition, $targetPlg, ($nrPossibleExtensions-1));
                                }
                                ?>
                                <h3><?php _e('Extensions', 'wp-admin-audit'); ?></h3>
                                <?php
                                $extLink = sprintf(
                                '<a class="wada-ui-button button button-primary" href="' . admin_url('admin.php?page=wp-admin-audit-settings&tab=tab-extensi') . '">%s</a>',
                                __('Show plugin extensions', 'wp-admin-audit')
                                );
                                ?>
                                <p><?php echo $extensionText; ?></p>
                                <p><?php echo $extLink; ?></p>
                            <?php
