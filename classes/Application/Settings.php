...
class WADA_Settings
{
    ...
    const INSTALLED_FIRST_ON_OR_BEFORE = 32;
    const INSTALLED_UPDATE_ON = 33;
    ...
    public static function getMetadataForAllSettings(){
        $metaDataArray = array(
            ...
            'setting'.self::INSTALLED_FIRST_ON_OR_BEFORE => (object) array(
                'settingId' => self::INSTALLED_FIRST_ON_OR_BEFORE,
                'defaultVal' =>  '',
                'getMethod' => 'getInstalledFirstOnOrBefore',
                'setMethod' => 'setInstalledFirstOnOrBefore',
                'externalType' => 'int',
                'storageType' => 'int',
                'field' => 'none'
            ),
            'setting'.self::INSTALLED_UPDATE_ON => (object) array(
                'settingId' => self::INSTALLED_UPDATE_ON,
                'defaultVal' =>  '',
                'getMethod' => 'getInstalledUpdateOn',
                'setMethod' => 'setInstalledUpdateOn',
                'onUpdateMethod' => 'onUpdateSetInstalledUpdateOn',
                'externalType' => 'int',
                'storageType' => 'int',
                'field' => 'none'
            )
    ...
    public static function doesSettingExist($settingId){
        ...
        global $wpdb;
        $query = 'SELECT COUNT(*) FROM '.WADA_Database::tbl_settings().' WHERE id = %d';
        $count = $wpdb->get_var($wpdb->prepare($query, $settingId));
        if($count > 0){
            return true;
        }
        return false;
    ...
    public static function getInstalledFirstOnOrBefore($default=null){
        $default = (is_null($default) || intval($default) <= 0) ? (time()) : (intval($default));
        return self::getSetting(self::INSTALLED_FIRST_ON_OR_BEFORE, $default);
    }

    public static function setInstalledFirstOnOrBefore($installedFirstOnOrBefore) {
        $installedFirstOnOrBefore = (is_null($installedFirstOnOrBefore) || intval($installedFirstOnOrBefore) <= 0) ? (time()) : (intval($installedFirstOnOrBefore));
        return self::setSetting(self::INSTALLED_FIRST_ON_OR_BEFORE, $installedFirstOnOrBefore);
    }

    public static function getInstalledUpdateOn($default=null){
        $default = (is_null($default) || intval($default) <= 0) ? (time()) : (intval($default));
        return self::getSetting(self::INSTALLED_UPDATE_ON, $default);
    }

    public static function setInstalledUpdateOn($installedUpdateOn){
        $installedUpdateOn = (is_null($installedUpdateOn) || intval($installedUpdateOn) <= 0) ? (time()) : (intval($installedUpdateOn));
        return self::setSetting(self::INSTALLED_UPDATE_ON, $installedUpdateOn);
    }

    public static function onUpdateSetInstalledUpdateOn(){
        return self::setInstalledUpdateOn(time());
    }
