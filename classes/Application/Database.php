...
class WADA_Database
{
    ...
    public static function isColExisting($tblName, $col){
        ...
        global $wpdb;
        $result = $wpdb->query(
            $wpdb->prepare( "SELECT * FROM information_schema.COLUMNS WHERE TABLE_NAME = %s AND COLUMN_NAME = %s AND TABLE_SCHEMA = DATABASE()", $tblName, $col )
        );
    ...
    public static function getColMaxLength($tblName, $col){
        global $wpdb;
        $maxLength = $wpdb->get_var(
            $wpdb->prepare( "SELECT CHARACTER_MAXIMUM_LENGTH FROM information_schema.COLUMNS WHERE TABLE_NAME = %s AND COLUMN_NAME = %s AND TABLE_SCHEMA = DATABASE()", $tblName, $col )
        );
        WADA_Log::debug('getColMaxLength tbl: '.$tblName.', col: '.$col.', $maxLength: '.$maxLength);
        return $maxLength;
    }
