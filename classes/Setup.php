...
class WADA_Setup
{
    ...
    public function createTablesIfNotExisting(){
        ...
        $tblSQLs[] = "CREATE TABLE IF NOT EXISTS ".$wpdb->prefix."wada_sensors (
                      ...
                      active_before TINYINT(1) NOT NULL DEFAULT 1,
    ...
    public function setupStandardSettingsIfNeeded(){
        ...
        self::infoLogDuringSetup('setupStandardSettingsIfNeeded');
        ...
        foreach ($settingsMetadata as $settingProperty => $metadata) {
            if(!WADA_Settings::doesSettingExist($metadata->settingId)){
                self::infoLogDuringSetup('setupStandardSettingsIfNeeded Setting not existing, add: settingProperty ' . $settingProperty . ': '.print_r($metadata, true));
                call_user_func(array('WADA_Settings', $metadata->setMethod), $metadata->defaultVal);
            }
            if(property_exists($metadata, 'onUpdateMethod') && method_exists('WADA_Settings', $metadata->onUpdateMethod)){
                self::infoLogDuringSetup('setupStandardSettingsIfNeeded Setting ' . $settingProperty . '  has onUpdateMethod -> call '. $metadata->onUpdateMethod);
                call_user_func(array('WADA_Settings', $metadata->onUpdateMethod));
            }
        }
    }
    ...
    protected function getCurrentDatabaseVersion(){
        ...
        return ($currentVersion.'.'.intval('138')); // attach build version as 4th level
    }
